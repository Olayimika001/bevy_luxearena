    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="py-3 sm:py-4 flex items-center justify-between gap-3 sm:gap-6">
        ...
        <router-link to="/" class="flex items-center flex-shrink-0">
          <img :src="logo" alt="Store Logo" class="h-20 sm:h-28 w-auto z-10" />
        </router-link>
        ...
        <div class="hidden md:flex flex-grow max-w-md">
          <div class="relative w-full flex items-center">
            <input 
              ...
              class="flex-grow border border-muted rounded-l px-3 sm:px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent"
            />
            
            <!-- Search Suggestions -->
            <div v-if="showSuggestions && searchInput && filteredSuggestions.length > 0" class="absolute top-full left-0 right-0 bg-white border border-t-0 border-muted rounded-b shadow-lg max-h-64 overflow-y-auto z-50">
              <button 
            ...
            <button 
              @click="performSearch"
              class="flex-shrink-0 bg-accent text-white px-3 sm:px-4 py-2 rounded-r hover:bg-opacity-90 transition"
              aria-label="Search"
            >
        ...
        <div class="flex items-center gap-3 sm:gap-6">
          <!-- Search Mobile -->
          <button @click="mobileSearchOpen = !mobileSearchOpen" class="lg:hidden p-1">
          ...
          <!-- Wishlist -->
          <button class="relative p-1" title="Wishlist">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <!-- Menu Mobile -->
          <button @click="mobileMenuOpen = !mobileMenuOpen" class="lg:hidden p-1">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- Mobile Search -->
      <div v-if="mobileSearchOpen" class="pb-4 lg:hidden border-t border-muted">
        <input 
          v-model="searchInput"
          @keyup.enter="performSearch"
          type="text" 
          placeholder="Search for bags..." 
          class="w-full border border-muted rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent"
        />
      </div>
