    ...
    <div class="grid lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2">
        <div v-if="cart.length > 0" class="space-y-4">
          ...
          <article v-for="item in cart" :key="item.id" class="bg-white rounded-lg p-6 flex gap-6 border border-muted hover:shadow-lg transition">
            <!-- Product Image -->
            <router-link :to="`/product/${item.id}`" class="flex-shrink-0 w-24 h-24 bg-muted rounded overflow-hidden">
              <img :src="item.image" :alt="item.name" class="w-full h-full object-cover hover:scale-110 transition-transform" />
            </router-link>

            <!-- Product Info -->
            <div class="flex-grow">
              <router-link :to="`/product/${item.id}`" class="font-serif font-bold text-lg hover:text-accent transition mb-1 block">
                {{ item.name }}
              </router-link>
              <p class="text-sm text-gray-600 mb-2">{{ item.color }}</p>
              <p class="text-sm text-gray-600 mb-3">{{ item.material }}</p>
              
              <!-- Stock Status -->
              <span v-if="item.inStock" class="text-xs text-green-600 font-bold">✓ In Stock</span>
              <span v-else class="text-xs text-red-600 font-bold">Out of Stock</span>
            </div>

            <!-- Quantity & Price -->
            <div class="flex flex-col items-end justify-between">
              <div class="text-right mb-2">
                <div v-if="item.discount > 0" class="space-y-1">
                  <span class="font-serif font-bold text-lg block">#{{ (item.price * (1 - item.discount / 100) * item.quantity).toFixed(2) }}</span>
                  <span class="text-xs line-through text-gray-500">#{{ (item.price * item.quantity).toFixed(2) }}</span>
                </div>
                <span v-else class="font-serif font-bold text-lg">#{{ (item.price * item.quantity).toFixed(2) }}</span>
              </div>
              
              <div class="flex items-center border border-muted rounded">
                <button @click="updateQuantity(item.id, item.quantity - 1)" class="px-3 py-1 hover:bg-muted transition">−</button>
                <span class="px-4 py-1">{{ item.quantity }}</span>
                <button @click="updateQuantity(item.id, item.quantity + 1)" class="px-3 py-1 hover:bg-muted transition">+</button>
              </div>

              <button @click="removeFromCart(item.id)" class="text-sm text-red-600 hover:text-red-700 transition mt-2">
                Remove
              </button>
            </div>
          </article>
          ...
          <div class="flex justify-between items-center mt-8 pt-8 border-t border-muted">
        ...
        <div class="bg-muted rounded-lg p-6 sticky top-4">
          ...
          <div class="space-y-4 mb-6 pb-6 border-b border-gray-300">
            ...
            <div class="flex justify-between">
              ...
              <span class="font-bold text-green-600">-#{{ totalDiscount.toFixed(2) }}</span>
            </div>
            <div class="flex justify-between">
          ...
          <router-link 
            to="/checkout" 
            :class="['btn-primary w-full text-center block', !cart.length && 'opacity-50 cursor-not-allowed pointer-events-none']"
          >
            Proceed to Checkout
          </router-link>
          ...
          <div v-if="cart.length > 0" class="mt-6 pt-6 border-t border-gray-300">
            ...
            <div class="flex gap-2">
              ...
              <button 
                ...
                class="px-4 py-2 bg-accent text-white rounded text-sm hover:bg-opacity-90 transition"
              >
                Apply
              </button>
            </div>
...
const tax = computed(() => {
  return ((cartTotal.value - totalDiscount.value) + shippingCost.value) * 0.08
})

const finalTotal = computed(() => {
  return (cartTotal.value - totalDiscount.value) + shippingCost.value + tax.value - appliedDiscount.value
})
...
const applyPromoCode = () => {
  ...
  if (promoCodes[code]) {
    appliedDiscount.value = (cartTotal.value - totalDiscount.value) * promoCodes[code]
